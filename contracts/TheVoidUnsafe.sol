...
contract TheVoidUnsafe {
    ...
    event NewShouter(address _shouter, uint256 _amount);
    event StakeWithdrawRequested(address _shouter, uint256 _amount);
    event StakeWithdrawn(address _shouter);
    event Transfer(address indexed from, address indexed to, uint256 value);

    // Storage
    mapping(bytes32 => mapping(uint256 => bool)) public isDisputed; //queryId -> timestamp -> value
    mapping(bytes32 => mapping(uint256 => address)) public shouterByTimestamp;
    mapping(address => StakeInfo) shouterDetails; //mapping from a persons address to their staking info
    ...
    uint256 public constant timeBasedReward = 5e17; // time based reward for a shouter for successfully submitting a value
    ...
    struct StakeInfo {
        ...
        uint256 shouterLastTimestamp; // timestamp of shouter's last reported value
        uint256 reportsSubmitted; // total number of reports submitted by shouter
    }
    ...
    /**
     * @dev Mock function for adding staking rewards. No rewards actually given to shouters
    ...
    /**
     * @dev Allows a shouter to submit stake
     * @param _amount amount of tokens to stake
     */
    function depositStake(uint256 _amount) external {
        StakeInfo storage _shouter = shouterDetails[msg.sender];
        if (_shouter.lockedBalance > 0) {
            if (_shouter.lockedBalance >= _amount) {
                _shouter.lockedBalance -= _amount;
            } else {
                require(
                    _transferFrom(
                        msg.sender,
                        address(this),
                        _amount - _shouter.lockedBalance
                    )
                );
                _shouter.lockedBalance = 0;
            }
        } else {
            require(_transferFrom(msg.sender, address(this), _amount));
        }
        _shouter.startDate = block.timestamp; // This resets their stake start date to now
        _shouter.stakedBalance += _amount;
        emit NewShouter(msg.sender, _amount);
    }
    ...
    /**
     * @dev Allows a shouter to request to withdraw their stake
    ...
    function requestStakingWithdraw(uint256 _amount) external {
        ...
        _shouter.startDate = block.timestamp;
        _shouter.lockedBalance += _amount;
        _shouter.stakedBalance -= _amount;
    ...
    /**
     * @dev Withdraws a shouter's stake
     */
    function withdrawStake() external {
        StakeInfo storage _s = shouterDetails[msg.sender];
        // Ensure shouter is locked and that enough time has passed
        require(block.timestamp - _s.startDate >= 7 days, "7 days didn't pass");
        require(_s.lockedBalance > 0, "shouter not locked for withdrawal");
    ...
    /**
     * @dev Returns the shouter for a given timestamp and queryId
     * @param _queryId bytes32 version of the queryId
     * @param _timestamp uint256 timestamp of report
     * @return address of data shouter
     */
    function getShouterByTimestamp(bytes32 _queryId, uint256 _timestamp)
        external
        view
        returns (address)
    {
        return shouterByTimestamp[_queryId][_timestamp];
    }
    ...
    /**
     * @dev Allows users to retrieve all information about a shouter
     * @param _shoutersAddress address of shouter inquiring about
     ...
     * @return uint shouter's last reported timestamp
     * @return uint total number of reports submitted by shouter
     * @return uint governance vote count when first staked
     * @return uint number of votes case by shouter when first staked
     * @return uint whether shouter is counted in totalShouters
     */
    function getShouterInfo(address _shoutersAddress)
        external
        view
        returns (
            uint256,
    ...
    {
        StakeInfo storage _shouter = shouterDetails[_shoutersAddress];
        return (
            _shouter.startDate,
            _shouter.stakedBalance,
            _shouter.lockedBalance,
            0, // reward debt
            _shouter.shouterLastTimestamp,
